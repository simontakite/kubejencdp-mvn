...
pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                withEnv(["APP_NAME=${APP_NAME}", "PROJECT_NAME=${PROJECT_NAME}"]) {
                    sh '''
                docker build -t 127.0.0.1:30400/${IMAGE_NAME}:${RELEASE_TAG} --build-arg APP_NAME=${IMAGE_NAME}  -f app/Dockerfile app/.
                '''
                }
            }
        }
        ...
        stage('Deploy') {
            ...
            cd $BASE_DIR/k8s/
            ./process_files.sh "${IMAGE_NAME}" "${DOCKER_PROJECT_NAMESPACE}/${IMAGE_NAME}:${RELEASE_TAG}" "./${
                    IMAGE_NAME
                }/" ${TIMESTAMP}
